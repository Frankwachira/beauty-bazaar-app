...
class AppDatabase {
  ...
  Future<Database> _openDb() async {
    ...
    try {
      if (kIsWeb) {
        ...
        // If factory is not ready, initialize FFI for desktop
        await getDatabasesPath();
      }
    } catch (_) {
      // getDatabasesPath failed because factory wasn't set (desktop)
      sqfliteFfiInit();
      databaseFactory = databaseFactoryFfi;
    }

    // Use a name on web; full path elsewhere
    final path = kIsWeb ? 'beautybazaar.db' : p.join(await getDatabasesPath(), 'beautybazaar.db');
      ...
      options: OpenDatabaseOptions(
        ...
        onUpgrade: (db, oldVersion, newVersion) async {
          ...
          if (oldVersion < 3) {
            ...
            try {
              await db.execute("ALTER TABLE bookings ADD COLUMN status TEXT NOT NULL DEFAULT 'active'");
            } catch (_) {}
            // Add role to users
            try {
  ...
  Future<void> _ensureOwnerExists(Database db) async {
    final owners = await db.rawQuery("SELECT COUNT(*) as c FROM users WHERE role = 'owner'");
    final ownersCount = (owners.first['c'] as int?) ?? 0;
    if (ownersCount == 0) {
      final firstUser = await db.query('users', orderBy: 'created_at ASC', limit: 1, columns: ['id']);
      if (firstUser.isNotEmpty) {
        final id = firstUser.first['id'] as int;
        await db.update('users', {'role': 'owner'}, where: 'id = ?', whereArgs: [id]);
      }
    }
  }
  ...
  Future<void> createUser(String username, String password) async {
    ...
    final ownersRows = await db.rawQuery("SELECT COUNT(*) as c FROM users WHERE role = 'owner'");
  ...
  Future<String?> getUserRole(String username) async {
    final db = await database;
    final rows = await db.query('users', columns: ['role'], where: 'username = ?', whereArgs: [username], limit: 1);
  ...
  Future<List<Booking>> findBookingsByPhone(String phone) async {
    final db = await database;
    final rows = await db.query('bookings', where: 'phone_number = ?', whereArgs: [phone], orderBy: 'appointment_dt DESC');
    return rows.map((m) => Booking.fromMap(m)).toList();
  }
}
